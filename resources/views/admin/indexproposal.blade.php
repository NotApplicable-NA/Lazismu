        ...
        <div class="container py-8 px-4">
            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Proposal <span class="badge bg-primary">{{ $allProposals->count() }}
                            Proposal</span></h5>
                </div>
                <div class="card-body p-3">
                    <table
                        class="table table-bordered table-striped text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            ...
                            @foreach ($proposals as $proposal)
                                <tr>
                                    <td class="text-center align-middle">
                                        ...
                                        @if (in_array($role, ['manager', 'bp']) && $proposal->status === 'Proses' && ($proposal->tampilkan_tanda ?? false))
                                            <span style="display:inline-block; width:10px; height:10px; border-radius:50%; background-color:blue;"></span>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        <span
                                            class="badge 
                            @if ($proposal->status == 'Masuk') bg-primary 
                            @elseif($proposal->status == 'Revisi') bg-warning 
                            @elseif($proposal->status == 'Proses') bg-warning 
                            @elseif($proposal->status == 'Diterima') bg-success 
                            ...
                            @else bg-dark @endif">
                                    ...
                                    <td>
                                        <div class="inline-flex space-x-4">
                                            <!-- read action -->
                                            <a href="{{ $detailUrl }}/{{ $proposal->id }}"
                                                class="flex items-center py-2 text-base font-medium text-gray-900 dark:text-white dark:hover:underline">
                        ...
                        <ul class="pagination mb-0 flex gap-2">
                            ...
                            @if ($proposals->onFirstPage())
                                <li class="page-item disabled"><span
                                        class="page-link text-gray-400 px-3 py-2">Previous</span></li>
                            @else
                                <li class="page-item">
                                    <a href="{{ $proposals->previousPageUrl() }}"
                                        class="page-link bg-white border px-3 py-2 hover:bg-gray-200">Previous</a>
                                </li>
                            @endif
                            ...
                            @foreach ($proposals->getUrlRange(1, $proposals->lastPage()) as $page => $url)
                                @if ($page == $proposals->currentPage())
                                    <li class="page-item active">
                                        <span
                                            class="page-link bg-blue-500 text-white px-3 py-2">{{ $page }}</span>
                                    </li>
                                @else
                                    <li class="page-item">
                                        <a href="{{ $url }}"
                                            class="page-link bg-white border px-3 py-2 hover:bg-gray-200">{{ $page }}</a>
                                    </li>
                                @endif
                            @endforeach
                            ...
                            @if ($proposals->hasMorePages())
                                <li class="page-item">
                                    <a href="{{ $proposals->nextPageUrl() }}"
                                        class="page-link bg-white border px-3 py-2 hover:bg-gray-200">Next</a>
                                </li>
                            @else
                                <li class="page-item disabled"><span
                                        class="page-link text-gray-400 px-3 py-2">Next</span></li>
                            @endif
                        </ul>
                    </nav>
                </div>
