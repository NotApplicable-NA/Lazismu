    ...
    <div class="container py-8 px-4">
            <!-- Header -->
                <header class="mb-4 mt-5">
                    <h1 class="fw-bold fs-5">Laporan Pertanggungjawaban</h1>
                    <hr class="custom-underline">
                </header>
            <!-- Proposal Table -->
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">LPJ <span class="badge bg-primary">{{$allLPJ->count()}} LPJ</span></h5>
                </div>
            ...
            <tbody>
               @foreach($lpjs as $lpj)
                <tr>
                    ...
                    <td>{{$lpj->judul_lpj}}</td>
                    <td>{{$lpj->dana_disetujui}}</td>
                    <td>{{$lpj->proposal->kontak}}</td>
                    <td>{{$lpj->tgl_masuk}}</td>
                    <!-- <td><span class="badge bg-info">Sudah</span></td> -->
                    <td>
                    <div class="inline-flex space-x-4">
                        <!-- read action -->
                        <a href="/admin/lpj/{{$lpj->id}}" class="flex items-center py-2 text-base font-medium text-gray-900 dark:text-white dark:hover:underline">
        ...
        <div class="card-footer d-flex flex-column flex-md-row justify-content-between align-items-center p-3">
            ...
            <p class="text-sm text-gray-600">
                Showing <span class="font-semibold">{{ $lpjs->firstItem() }}</span> 
                to <span class="font-semibold">{{ $lpjs->lastItem() }}</span> 
                of <span class="font-semibold">{{ $lpjs->total() }}</span> entries
            </p>
            ...
            <nav>
                <ul class="pagination mb-0 flex gap-2">
                    ...
                    @else
                        <li class="page-item">
                            <a href="{{ $lpjs->previousPageUrl() }}" class="page-link bg-white border px-3 py-2 hover:bg-gray-200">Previous</a>
                        </li>
                    @endif

                    {{-- Halaman --}}
                    @foreach ($lpjs->getUrlRange(1, $lpjs->lastPage()) as $page => $url)
                        @if ($page == $lpjs->currentPage())
                    ...
                    @if ($lpjs->hasMorePages())
                        <li class="page-item">
                            <a href="{{ $lpjs->nextPageUrl() }}" class="page-link bg-white border px-3 py-2 hover:bg-gray-200">Next</a>
                        </li>
                    @else
